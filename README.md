...
- 32×64처럼 긴 타일은 같은 배율로 64×128 크기로 보여주고, 64×64 이상 또는 긴 시트도 비율 유지 썸네일로 보여줍니다.
- 단색 중심의 미니멀 격자 UI에서 여러 이미지를 선택할 수 있습니다.
- 선택, 파일, 복사는 왼쪽 패널에, 크기, 투명화, 팔레트, 보정은 오른쪽 패널에 배치됩니다.
...
- `선택 변환`을 누르면 현재 선택한 이미지만 팔레트 색으로 실제 변환합니다.
- 실제 변환 전 확인 팝업을 띄우고, 원본은 `tools/asset_browser/palette_backups/` 아래에 자동 백업합니다.
- 펼친 폴더 구획의 `↪ 폴더` 버튼을 누르면 브라우저 위치가 해당 폴더로 이동해 그 안의 이미지들만 다시 스캔합니다.
- 스캔 폴더, 기본 프롬프트 템플릿, 아트 스타일 토큰 파일을 Finder에서 바로 볼 수 있습니다.
- 선택한 상대경로 목록을 `.txt` 파일로 저장합니다.
- 선택한 이미지 파일을 확인 후 실제 삭제할 수 있습니다.
- 그리드에서 대상 이미지 하나를 선택한 뒤 `파일 교체`를 누르면, 선택한 다른 이미지 파일의 내용으로 대상 파일을 바꾸고 원본 파일은 삭제합니다. 대상 파일명과 확장자는 유지됩니다.
- `Godot 전체 반영` 버튼은 `godot --headless --editor --quit`를 실행해 같은 파일명으로 교체된 이미지를 포함한 프로젝트 전체 이미지 import 메타데이터를 갱신합니다.
- 선택한 이미지를 `32x32`, `32x64`, `64x64` 같은 지정 크기로 원본 파일에 적용합니다. 새 파일은 만들지 않으며, 변경 전 원본은 `tools/asset_browser/resize_backups/` 아래에 자동 백업합니다.
...
- 상세보기 도구막대에서 펜, 지우개, 직선, 채우기, 스포이드, 외곽 투명화를 사용할 수 있습니다.
...
- 상세 편집창에서 선택 영역을 새 캔버스 범위로 확장하고 바깥을 투명 패딩으로 채워 원본에 저장할 수 있습니다.
- 상세 편집창에서 지정한 색을 이미지 전체에서 다른 색으로 한 번에 치환할 수 있으며 허용 범위와 알파값은 유지됩니다.
