...
- 단색 중심의 밀집 격자 UI에서 한 화면에 더 많은 이미지를 보고 선택할 수 있습니다.
- 선택, 파일, 복사는 왼쪽 패널에, 크기, 투명화, 팔레트, 보정은 오른쪽 패널에 배치됩니다.
- 스캔 루트 안의 폴더와 하위 폴더를 들여쓰기한 트리로 보여주고, 각 폴더와 이미지 크기 그룹을 따로 여닫을 수 있습니다.
- 시작할 때는 첫 이미지까지의 폴더 경로와 첫 크기 그룹만 엽니다. 나머지 가지는 필요할 때 열어 썸네일과 투명도를 읽으므로 대규모 에셋 폴더의 초기 로딩을 줄입니다.
- 32x32 에셋은 98px 기본 셀에 밀집 배치하고, 대형 시트는 최대 96px 요약 미리보기로 표시해 파일별 영역이 흔들리지 않습니다.
...
- 폴더 행의 `↪ 이 폴더만 보기` 버튼을 누르면 브라우저 위치가 해당 폴더로 이동해 그 안의 이미지들만 다시 스캔합니다.
- 상단 `↑ 상위` 버튼이나 `Cmd+↑`로 현재 스캔 폴더의 부모 폴더로 한 단계 이동합니다.
...
- 보정 강도는 `-100%`에서 `100%`까지 1% 단위 가로 슬라이더로 선택합니다.
...
- 메인 브라우저는 평면형 버튼, 낮은 대비의 패널, 짧은 기능명으로 구성되며, 기능 버튼에 마우스나 트랙패드 포인터를 올리면 동작 설명과 단축키가 툴팁으로 표시됩니다.
- 이미지 위를 누른 채 드래그하면 지나가는 이미지들이 빠르게 선택됩니다. 가장자리로 드래그하면 목록이 자동으로 조금씩 스크롤됩니다.
- 이미지를 더블클릭하면 원본 픽셀 좌표 기준의 상세 편집창이 열립니다.
- 상세 편집창은 Aseprite와 비슷한 좌측 도구막대, 중앙 캔버스, 상단 문서·줌 도구막대, 우측 `색상`/`선택·저장` 탭으로 구성됩니다. 메인 브라우저와 같은 평면형 버튼과 hover 툴팁을 사용하며, 현재 도구는 옅은 teal 배경으로 표시하고 `원본 저장`만 강조색으로 구분합니다.
- 펜, 지우개, 직선, 채우기, 스포이드, 이동, 외곽 투명화를 지원하고 현재 도구와 조작 방법을 상단에 표시합니다.
- 마우스 휠로 줌하고, 트랙패드 두 손가락 또는 `Space+드래그`로 캔버스를 이동하며, `F`로 화면에 맞춥니다.
- 800% 이상 줌에서 실제 그려진 픽셀에만 격자를 표시하며, 완전히 투명한 픽셀에는 격자선을 그리지 않습니다. 투명 영역은 픽셀보다 큰 중간 회색 체커로 보여 실색과 구분하고, 상단 `투명 보기` 또는 `D`로 체커·밝은 배경·어두운 배경을 즉시 전환할 수 있습니다.
- 편집 작업은 `Cmd+Z`/`Shift+Cmd+Z`로 실행 취소·다시 실행하고, 미저장 상태로 닫을 때 저장 여부를 확인합니다.
...
- 상세 편집창의 `Cmd+S` 또는 `Ctrl+S`는 원본을 저장하고, `Cmd+E` 또는 `Ctrl+E`는 현재 크롭 영역을 PNG로 내보냅니다.
- 상세 편집창에서 선택 영역을 새 캔버스 범위로 확장하고 바깥을 투명 패딩으로 채워 원본에 저장할 수 있습니다.
- 상세 편집창에서 지정한 색을 이미지 전체에서 다른 색으로 한 번에 치환할 수 있으며 허용 범위와 알파값은 유지됩니다. `치환할 원본색`은 HEX 입력, 기존 색상 선택기, 원본색 전용 스포이드로 지정할 수 있고, 상단 `⌖` 버튼으로 결과색 스포이드를 즉시 활성화할 수 있습니다. 투명 픽셀 선택이나 일치 색 0개는 이유를 안내합니다.

상세 편집창 주요 단축키:

- `M`: 선택/크롭, `B`: 펜, `E`: 지우개, `L`: 직선
- `G`: 채우기, `I`: 스포이드, `H`: 이동, `Space`: 임시 이동
- `1`~`6`: 100%~3200% 줌, `F`: 화면 맞춤, `D`: 투명 배경 보기 전환
- `Cmd+S`: 원본 저장, `Cmd+E`: 현재 크롭 영역 PNG 내보내기
- `Q`: 현재 영역을 대기 목록에 추가
