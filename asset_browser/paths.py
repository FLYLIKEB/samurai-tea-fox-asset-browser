...
def paint_backup_root(project_root: Path | None = None) -> Path:
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    if project_root is None:
        return Path.cwd().resolve() / "tools" / "asset_browser" / "paint_backups" / stamp
    return project_root / "tools" / "asset_browser" / "paint_backups" / stamp

def adjustment_backup_root(project_root: Path | None = None) -> Path:
    stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    if project_root is None:
